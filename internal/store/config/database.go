...
func DefaultPostgreSQLConfig() *Config {
	return &Config{
		...
		ConnMaxIdleTime: 30 * time.Minute, // Keep idle connections longer for PostgreSQL
...
func NewDB(config *Config) (*DB, error) {
	...
	defer cancel()
	if err := db.SetStatementTimeouts(ctx); err != nil {
		// Don't fail on timeout setting errors, just log them
		fmt.Printf("Warning: Failed to set statement timeouts: %v\n", err)
	}
...
// CreateStore creates a Store interface implementation based on the database type
func (db *DB) CreateStore() (interface{}, error) {
	// Import the store package dynamically to avoid circular dependency
	// We return interface{} to avoid importing store package in database package

	switch db.dbType {
	case DatabaseTypePostgreSQL:
		// Create PostgresStore with existing configuration
		return createPostgresStore(db.config)
	case DatabaseTypeSQLite:
		// Create SQLiteStore with existing configuration
		return createSQLiteStore(db.config)
	default:
		return nil, fmt.Errorf("unsupported database type: %s", db.dbType)
	}
}

// These functions will be implemented by importing the store package
// This approach avoids circular dependencies
var (
	createPostgresStore func(*Config) (interface{}, error)
	createSQLiteStore   func(*Config) (interface{}, error)
)
...
// SetStatementTimeouts configures database-specific statement timeouts
func (db *DB) SetStatementTimeouts(ctx context.Context) error {
	if db.IsPostgreSQL() {
		return db.setPostgreSQLTimeouts(ctx)
	}
	return db.setSQLiteTimeouts(ctx)
}

// setPostgreSQLTimeouts applies PostgreSQL-specific timeout settings
func (db *DB) setPostgreSQLTimeouts(ctx context.Context) error {
	timeoutSettings := []string{
		fmt.Sprintf("SET statement_timeout = %d", int(DefaultStatementTimeout.Milliseconds())),
		fmt.Sprintf("SET lock_timeout = %d", int(15*time.Second.Milliseconds())),
		fmt.Sprintf("SET idle_in_transaction_session_timeout = %d", int(60*time.Second.Milliseconds())),
	}

	for _, setting := range timeoutSettings {
		if _, err := db.ExecContext(ctx, setting); err != nil {
			fmt.Printf("Warning: Failed to set PostgreSQL timeout '%s': %v\n", setting, err)
		}
	}
...
// setSQLiteTimeouts applies SQLite-specific timeout settings
func (db *DB) setSQLiteTimeouts(ctx context.Context) error {
	timeoutSettings := []string{
		fmt.Sprintf("PRAGMA busy_timeout = %d", int(DefaultStatementTimeout.Milliseconds())),
	}

	for _, setting := range timeoutSettings {
		if _, err := db.ExecContext(ctx, setting); err != nil {
			fmt.Printf("Warning: Failed to set SQLite timeout '%s': %v\n", setting, err)
		}
	}
