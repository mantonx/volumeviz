...
-- Description: Add comprehensive indexing strategy for query performance optimization (SQLite version)
-- Up Migration
...
CREATE INDEX IF NOT EXISTS idx_volumes_is_active ON volumes(is_active) WHERE is_active = true;
...
-- We use regular indexes on JSON columns instead
CREATE INDEX IF NOT EXISTS idx_volumes_labels ON volumes(labels);
CREATE INDEX IF NOT EXISTS idx_volumes_options ON volumes(options);
...
CREATE INDEX IF NOT EXISTS idx_volume_sizes_scan_method ON volume_sizes(scan_method);
CREATE INDEX IF NOT EXISTS idx_volume_sizes_is_valid ON volume_sizes(is_valid) WHERE is_valid = true;

-- Composite index for latest scan results
CREATE INDEX IF NOT EXISTS idx_volume_sizes_volume_latest 
...
CREATE INDEX IF NOT EXISTS idx_containers_is_active ON containers(is_active) WHERE is_active = true;
CREATE INDEX IF NOT EXISTS idx_containers_started_at ON containers(started_at);

-- Regular index for container labels (SQLite doesn't support GIN)
CREATE INDEX IF NOT EXISTS idx_containers_labels ON containers(labels);
...
CREATE INDEX IF NOT EXISTS idx_volume_mounts_is_active ON volume_mounts(is_active) WHERE is_active = true;

-- Composite index for active mounts
CREATE INDEX IF NOT EXISTS idx_volume_mounts_active 
...
CREATE INDEX IF NOT EXISTS idx_volume_metrics_total_size ON volume_metrics(total_size);

-- Composite index for time-series queries
CREATE INDEX IF NOT EXISTS idx_volume_metrics_timeseries 
...
-- Regular index for system health metadata (SQLite doesn't support GIN)
CREATE INDEX IF NOT EXISTS idx_system_health_metadata ON system_health(metadata);
...
CREATE INDEX IF NOT EXISTS idx_scan_cache_is_valid ON scan_cache(is_valid) WHERE is_valid = true;

-- Composite index for valid cache entries
CREATE INDEX IF NOT EXISTS idx_scan_cache_valid 
