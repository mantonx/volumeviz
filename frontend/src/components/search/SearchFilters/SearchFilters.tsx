...
export const SearchFilters: React.FC<SearchFiltersProps> = ({ onFilterChange }) => {
  ...
  const updateFilters = useCallback((updates: any) => {
    setFilters(prev => {
      ...
      if (newFilters.mimeTypes?.length) urlUpdates.mime = newFilters.mimeTypes;
      if (newFilters.sizeRange?.min) urlUpdates.minSize = newFilters.sizeRange.min;
      if (newFilters.sizeRange?.max) urlUpdates.maxSize = newFilters.sizeRange.max;
      if (newFilters.timeRange?.from) urlUpdates.mtimeFrom = newFilters.timeRange.from;
      if (newFilters.timeRange?.to) urlUpdates.mtimeTo = newFilters.timeRange.to;
      if (newFilters.booleanFilters?.hasGps !== undefined) urlUpdates.hasGps = newFilters.booleanFilters.hasGps;
      if (newFilters.booleanFilters?.hasSubs !== undefined) urlUpdates.hasSubs = newFilters.booleanFilters.hasSubs;
  ...
  const categorizedMimeTypes = useMemo(() => {
    const categories: Record<string, Array<{ value: string; label: string; fileCount: number }>> = {
      video: [],
      audio: [],
      image: [],
      document: [],
    ...
    dynamicMimeTypes.forEach(mimeType => {
      ...
      switch (type) {
        ...
        case 'application':
          ...
          if (mimeType.value.includes('pdf') || mimeType.value.includes('document') || 
              mimeType.value.includes('word') || mimeType.value.includes('excel') ||
              mimeType.value.includes('powerpoint') || mimeType.value.includes('text')) {
            categories.document.push(mimeType);
          } else if (mimeType.value.includes('zip') || mimeType.value.includes('rar') ||
                     mimeType.value.includes('tar') || mimeType.value.includes('7z') ||
  ...
  const updateMediaKind = useCallback((mediaKind: string) => {
    updateFilters({
  ...
  const updateMimeTypes = useCallback((mimeType: string, checked: boolean) => {
    updateFilters({
      mimeTypes: checked
  ...
  const updateSizeRange = useCallback((range: { min?: number; max?: number }) => {
    setFilters(prev => ({
      ...prev,
      sizeRange: range,
    }));
  ...
  const updateCustomSize = useCallback((type: 'min' | 'max', value: string) => {
  ...
  const updateTimeRange = useCallback((type: 'from' | 'to', value: string) => {
    setFilters(prev => ({
      ...prev,
      timeRange: {
  ...
  const updateDurationRange = useCallback((type: 'min' | 'max', value: string) => {
  ...
  const updateDimensions = useCallback((dimension: 'width' | 'height', type: 'min' | 'max', value: string) => {
  ...
  const updateBooleanFilter = useCallback((key: 'hasGps' | 'hasSubs' | 'hashPresent', value: boolean | undefined) => {
    setFilters(prev => ({
      ...prev,
      booleanFilters: {
  ...
  const removeFilter = useCallback((filterType: string, filterKey?: string) => {
    setFilters(prev => {
      const updated = { ...prev };
      ...
      switch (filterType) {
        ...
        case 'mimeType':
          if (filterKey) {
            updated.mimeTypes = updated.mimeTypes.filter(type => type !== filterKey);
          }
          break;
        case 'sizeRange':
          updated.sizeRange = {};
        ...
        case 'booleanFilter':
          if (filterKey && updated.booleanFilters) {
            delete updated.booleanFilters[filterKey as keyof typeof updated.booleanFilters];
          }
          break;
      }
  ...
  const isVideoOrAudio = filters.mediaKind === 'video' || filters.mediaKind === 'audio';
  const isImageOrVideo = filters.mediaKind === 'image' || filters.mediaKind === 'video';
    ...
    <Card className="p-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
        ...
        <div className="space-y-3">
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Media Type</label>
          <select
        ...
        <div className="space-y-3">
          ...
          <select
            value={(() => {
              ...
              const currentPreset = SIZE_PRESETS.find(p => 
                p.min === filters.sizeRange?.min && p.max === filters.sizeRange?.max
              );
          ...
          <div className="grid grid-cols-2 gap-2 mt-2">
            ...
            <input
              type="number"
              placeholder="Max (MB)"
              value={filters.sizeRange.max ? Math.round(filters.sizeRange.max / (1024 * 1024)) : ''}
        ...
        {/* Time Range Filter */}
        <div className="space-y-3">
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Modified Date</label>
          <div className="grid grid-cols-1 gap-2">
            <input
              type="date"
              placeholder="From"
              value={filters.timeRange.from ? filters.timeRange.from.split('T')[0] : ''}
              onChange={(e) => updateTimeRange('from', e.target.value ? `${e.target.value}T00:00:00Z` : '')}
              className="px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 placeholder-gray-400 dark:placeholder-gray-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
            />
            <input
              type="date"
              placeholder="To"
              value={filters.timeRange.to ? filters.timeRange.to.split('T')[0] : ''}
              onChange={(e) => updateTimeRange('to', e.target.value ? `${e.target.value}T23:59:59Z` : '')}
              className="px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 placeholder-gray-400 dark:placeholder-gray-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
            />
          </div>
        </div>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              <input
                type="number"
                placeholder="Min"
                value={filters.durationRange.min ? Math.round(filters.durationRange.min / 1000) : ''}
              ...
              <input
                type="number"
                placeholder="Max"
                value={filters.durationRange.max ? Math.round(filters.durationRange.max / 1000) : ''}
        ...
        {showDimensionsFilter && (
          <div className="space-y-3">
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Dimensions (pixels)</label>
            <div className="space-y-2">
              <div className="grid grid-cols-2 gap-2">
                <input
                  ...
                  onChange={(e) => updateDimensions('width', 'min', e.target.value)}
                  className="px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 placeholder-gray-400 dark:placeholder-gray-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                />
                <input
                  type="number"
                  ...
                  onChange={(e) => updateDimensions('width', 'max', e.target.value)}
                  className="px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 placeholder-gray-400 dark:placeholder-gray-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                />
              </div>
              <div className="grid grid-cols-2 gap-2">
                <input
                  ...
                  onChange={(e) => updateDimensions('height', 'min', e.target.value)}
                  className="px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 placeholder-gray-400 dark:placeholder-gray-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                />
                <input
                  type="number"
                  ...
                  onChange={(e) => updateDimensions('height', 'max', e.target.value)}
                  className="px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 placeholder-gray-400 dark:placeholder-gray-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                />
              </div>
            </div>
          ...
          <div className="space-y-2">
            ...
            {(filters.mediaKind === 'image' || !filters.mediaKind) && (
              <label className="flex items-center">
                <input
                  ...
                  onChange={(e) => updateBooleanFilter('hasGps', e.target.checked ? true : undefined)}
                  className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                />
                <span className="ml-2 text-sm text-gray-700 dark:text-gray-300">Has GPS coordinates</span>
              </label>
            )}
            ...
            {(filters.mediaKind === 'video' || !filters.mediaKind) && (
              <label className="flex items-center">
                <input
                  ...
                  onChange={(e) => updateBooleanFilter('hasSubs', e.target.checked ? true : undefined)}
                  className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                />
                <span className="ml-2 text-sm text-gray-700 dark:text-gray-300">Has subtitles</span>
              </label>
            )}
            ...
            <label className="flex items-center">
              <input
                ...
                onChange={(e) => updateBooleanFilter('hashPresent', e.target.checked ? true : undefined)}
                className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
              />
              <span className="ml-2 text-sm text-gray-700 dark:text-gray-300">Has file hash</span>
            </label>
          </div>
        </div>
      </div>
      ...
      <div className="mt-6 pt-6 border-t border-gray-200">
        <details className="space-y-4">
          <summary className="text-sm font-medium text-gray-700 dark:text-gray-300 cursor-pointer">
            ...
            {metadataLoading && <span className="ml-2 text-xs text-gray-500">(loading...)</span>}
            {metadataError && <span className="ml-2 text-xs text-red-500">(error loading)</span>}
          </summary>
          ...
          {dynamicMimeTypes.length > 0 ? (
            <div className="space-y-4">
              ...
              <div className="flex gap-2 flex-wrap">
                <button
                  onClick={() => {
                    ...
                    setFilters(prev => ({ ...prev, mimeTypes: allMimeTypes }));
                  }}
                ...
                <button
                  onClick={() => setFilters(prev => ({ ...prev, mimeTypes: [] }))}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {Object.entries(categorizedMimeTypes).map(([category, mimeTypes]) => {
                  ...
                  const selectedInCategory = mimeTypes.filter(mt => filters.mimeTypes.includes(mt.value)).length;
                    ...
                    <div key={category} className="border border-gray-200 rounded-lg p-3">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            onClick={() => {
                              const categoryMimeTypes = mimeTypes.map(mt => mt.value);
                              const allSelected = categoryMimeTypes.every(mt => filters.mimeTypes.includes(mt));
                              
                              if (allSelected) {
                                // Deselect all in category
                                setFilters(prev => ({
                                  ...prev,
                                  mimeTypes: prev.mimeTypes.filter(mt => !categoryMimeTypes.includes(mt))
                                }));
                              } else {
                                // Select all in category
                                setFilters(prev => ({
                                  ...prev,
                                  mimeTypes: [...new Set([...prev.mimeTypes, ...categoryMimeTypes])]
                                }));
                              }
                            }}
                          ...
                          >
                            {selectedInCategory === totalInCategory ? 'None' : 'All'}
                          </button>
                        </div>
                      </div>
                      ...
                      <div className="space-y-1 max-h-32 overflow-y-auto">
                        {mimeTypes.slice(0, 10).map(mimeType => (
                          <label key={mimeType.value} className="flex items-center text-xs">
                            <input
                              type="checkbox"
                              checked={filters.mimeTypes.includes(mimeType.value)}
                              onChange={(e) => updateMimeTypes(mimeType.value, e.target.checked)}
                              className="rounded border-gray-300 text-blue-600 focus:ring-blue-500 mr-2"
                            />
                            <span className="flex-1 text-gray-700 dark:text-gray-300" title={mimeType.value}>
                              {mimeType.label}
                            </span>
      ...
      {(filters.mediaKind || filters.mimeTypes.length > 0 || 
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {filters.mimeTypes.length > 0 && (
              <>
                {filters.mimeTypes.map(mimeType => {
                  ...
                  const mimeTypeData = dynamicMimeTypes.find(mt => mt.value === mimeType);
                  const displayLabel = mimeTypeData?.label || mimeType;
                  
                  return (
                    <span key={mimeType} className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800">
                      {displayLabel}
                      <button
            ...
            {(filters.durationRange?.min || filters.durationRange?.max) && (
              <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-indigo-100 text-indigo-800">
                Duration: {filters.durationRange.min ? Math.round(filters.durationRange.min / 1000) + 's' : ''} - {filters.durationRange.max ? Math.round(filters.durationRange.max / 1000) + 's' : ''}
                <button
            ...
            {(filters.dimensionsRange?.width?.min || filters.dimensionsRange?.width?.max || 
              filters.dimensionsRange?.height?.min || filters.dimensionsRange?.height?.max) && (
            ...
            {Object.entries(filters.booleanFilters || {}).map(([key, value]) => 
              value && (
                <span key={key} className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-teal-100 text-teal-800">
                  {key === 'hasGps' ? 'GPS' : key === 'hasSubs' ? 'Subtitles' : 'Hash'}
                  <button
