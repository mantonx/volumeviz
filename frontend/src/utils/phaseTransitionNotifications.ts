...
export function formatPhaseTransition(transition: PhaseTransition): {
  ...
  const fromConfig = transition.fromPhase
    ? PHASE_DISPLAY_CONFIG[
        transition.fromPhase as keyof typeof PHASE_DISPLAY_CONFIG
      ]
    : null;
  const toConfig =
    PHASE_DISPLAY_CONFIG[
      transition.toPhase as keyof typeof PHASE_DISPLAY_CONFIG
    ];
  ...
  if (transition.fromPhase) {
    ...
    message =
      TRANSITION_MESSAGES.started[
        transition.toPhase as keyof typeof TRANSITION_MESSAGES.started
      ];
  ...
  if (transition.metadata?.filesProcessed) {
    stats.push(
      `${transition.metadata.filesProcessed.toLocaleString()} files processed`,
    );
  }
  ...
  if (transition.metadata?.performance?.averageSpeed) {
    stats.push(
      `${transition.metadata.performance.averageSpeed.toFixed(1)} files/sec avg`,
    );
  }
...
export class PhaseTransitionDetector {
  private lastPhaseStates = new Map<
    string,
    { phase: string; status: string; startTime: Date }
  >();
  ...
  updateProgress(
    ...
    if (!lastState) {
      ...
      if (phaseStatus === 'running') {
        transition = createPhaseTransition(
          scanId,
          volumeId,
          null,
          currentPhase,
    ...
    if (
      phaseStatus === 'running' &&
      (!lastState || lastState.phase !== currentPhase)
    ) {
...
export function getPhaseCompletionSummary(
  ...
  const config =
    PHASE_DISPLAY_CONFIG[phase as keyof typeof PHASE_DISPLAY_CONFIG];
  ...
  else if (phase === 'filesystem_indexing' && duration < 300000)
    performance = 'excellent';
  else if (phase === 'filesystem_indexing' && duration < 900000)
    performance = 'good';
  else if (phase === 'media_enrichment' && duration < 600000)
    performance = 'excellent';
  else if (phase === 'media_enrichment' && duration < 1800000)
    performance = 'good';
  ...
  if (metadata?.performance?.averageSpeed) {
    stats.push(
      `${metadata.performance.averageSpeed.toFixed(1)} files/sec average`,
    );
  }
