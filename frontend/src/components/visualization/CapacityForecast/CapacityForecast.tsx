...
export const CapacityForecast: React.FC<CapacityForecastProps> = ({
  ...
  const filteredVolumes = useMemo(() => {
    const volumesToShow = selectedVolumes.length > 0 ? selectedVolumes : 
      Array.from(new Set(historicalData.map(d => d.volumeId)));
    
    return volumesToShow.filter(volumeId => volumeVisibility[volumeId] !== false);
  ...
  const forecastSummaries: ForecastSummary[] = useMemo(() => {
    return filteredVolumes.map(volumeId => {
      const volumePredictions = predictions.filter(p => p.volumeId === volumeId);
      const historicalPoints = volumePredictions.filter(p => p.actualSize !== undefined);
      const forecastPoints = volumePredictions.filter(p => p.actualSize === undefined);

      if (!historicalPoints.length || !forecastPoints.length) return null;

      const currentSize = historicalPoints[historicalPoints.length - 1].actualSize!;
      const forecast30d = forecastPoints.find(p => {
        const daysDiff = (p.date.getTime() - historicalPoints[historicalPoints.length - 1].date.getTime()) / (1000 * 60 * 60 * 24);
        return Math.abs(daysDiff - 30) < 1;
      })?.predictedSize || currentSize;

      const forecast90d = forecastPoints.find(p => {
        const daysDiff = (p.date.getTime() - historicalPoints[historicalPoints.length - 1].date.getTime()) / (1000 * 60 * 60 * 24);
        return Math.abs(daysDiff - 90) < 1;
      })?.predictedSize || currentSize;

      const forecast365d = forecastPoints.find(p => {
        const daysDiff = (p.date.getTime() - historicalPoints[historicalPoints.length - 1].date.getTime()) / (1000 * 60 * 60 * 24);
      ...
      if (capacityLimit && forecast90d > capacityLimit * 0.8) {
        const severity = forecast90d > capacityLimit ? 'critical' : 
                        forecast90d > capacityLimit * 0.9 ? 'high' : 'medium';
        
        capacityAlert = {
          volumeId,
          volumeName,
          severity,
          type: forecast90d > capacityLimit ? 'exceeding_limit' : 'approaching_limit',
          message: `Volume ${volumeName} is ${severity === 'critical' ? 'predicted to exceed' : 'approaching'} capacity limit`,
          threshold: capacityLimit,
          predicted_date: forecastPoints.find(p => p.predictedSize > capacityLimit)?.date || new Date(),
          recommended_action: severity === 'critical' ? 'Immediate action required' : 'Monitor closely',
        };

        // Calculate time to capacity
        const exceedingPoint = forecastPoints.find(p => p.predictedSize > capacityLimit);
        if (exceedingPoint) {
          timeToCapacity = Math.floor((exceedingPoint.date.getTime() - Date.now()) / (1000 * 60 * 60 * 24));
        }
      ...
      return {
        ...
        growthTrend: forecast90d > currentSize * 1.1 ? 'linear' as const : 'declining' as const,
        capacityAlert,
        timeToCapacity,
      };
  ...
  const handleForecastPeriodChange = useCallback((days: number) => {
    ...
    return (
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-lg shadow-lg p-3">
        ...
        {props.payload.map((entry: any, index: number) => {
          if (!entry.dataKey.includes('_actual') && !entry.dataKey.includes('_predicted')) return null;
          
          const volumeId = entry.dataKey.replace('_actual', '').replace('_predicted', '');
          const summary = forecastSummaries.find(s => s.volumeId === volumeId);
          ...
          return (
            <div key={index} className="flex items-center gap-2 text-sm">
              ...
              <span className="text-gray-600 dark:text-gray-300">
                {summary.volumeName} ({isActual ? 'Actual' : 'Predicted'}): {formatBytes(value)}
              </span>
            </div>
          );
        })}
    ...
    return (
      <div className={clsx('bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6', className)}>
        <div className="text-center py-8">
          ...
          <p className="text-gray-500 dark:text-gray-400 mb-2">No historical data for forecasting</p>
          <p className="text-sm text-gray-400 dark:text-gray-500">
            At least 3 historical data points are needed for capacity forecasting
          </p>
        </div>
      </div>
    );
              ...
              .map(summary => (
                <div
                  ...
                  className={clsx(
                    ...
                    summary.capacityAlert!.severity === 'critical' ? 'bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800' :
                    summary.capacityAlert!.severity === 'high' ? 'bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800' :
                    'bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800'
                  )}
                >
                  <AlertTriangle className={clsx(
                    'w-4 h-4',
                    summary.capacityAlert!.severity === 'critical' ? 'text-red-600' :
                    summary.capacityAlert!.severity === 'high' ? 'text-orange-600' : 'text-yellow-600'
                  )} />
      ...
      <div className="px-6 py-4">
        <ResponsiveContainer width="100%" height={height}>
          <ComposedChart data={chartData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
            ...
            {showThresholds && Object.entries(capacityLimits).map(([volumeId, limit]) => {
              ...
              return (
                <ReferenceLine 
                  ...
                  strokeDasharray="2 2"
                  strokeOpacity={0.7}
                  label={{ value: `Capacity Limit`, position: 'topRight', fontSize: 12 }}
                />
              );
            })}
          </ComposedChart>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
              {forecastSummaries.map(summary => (
                <tr key={summary.volumeId} className="text-gray-900 dark:text-white">
                  <td className="py-2 font-medium">{summary.volumeName}</td>
                  <td className="py-2 font-mono">{formatBytes(summary.currentSize)}</td>
                  <td className="py-2 font-mono text-blue-600">{formatBytes(summary.forecastSize30d)}</td>
                  <td className="py-2 font-mono text-blue-600">{formatBytes(summary.forecastSize90d)}</td>
                  <td className="py-2 font-mono text-blue-600">{formatBytes(summary.forecastSize365d)}</td>
                  <td className="py-2">
                    <div className="flex items-center gap-1">
                      <TrendingUp className={clsx(
                        'w-4 h-4',
                        summary.growthTrend === 'linear' ? 'text-green-500' : 'text-gray-400'
                      )} />
                  ...
                  {showAccuracy && (
                    <td className="py-2">
                      <span className={clsx(
                        ...
                        summary.model.accuracy > 0.8 ? 'bg-green-100 text-green-800' :
                        summary.model.accuracy > 0.6 ? 'bg-yellow-100 text-yellow-800' :
                  ...
                  <td className="py-2">
                    {summary.capacityAlert ? (
                      <span className={clsx(
                        'px-2 py-1 rounded-full text-xs',
                        summary.capacityAlert.severity === 'critical' ? 'bg-red-100 text-red-800' :
                        summary.capacityAlert.severity === 'high' ? 'bg-orange-100 text-orange-800' :
