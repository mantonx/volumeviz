...
export const WebSocketConnectionTest: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const handleDisconnect = (data: any) => logEvent('Disconnected', `Code: ${data?.code}`);
    const handleError = (data: any) => logEvent('Error', data?.error || 'Unknown error');
    const handleReconnecting = (data: any) => logEvent('Reconnecting', `Attempt ${data?.attempt}`);
  ...
  const testErrorHandling = async () => {
    ...
    try {
      ...
      const success = ws.send({ type: 'invalid_test_message', data: { test: true } });
  ...
  const getResultColor = (result: 'pass' | 'fail' | 'testing' | null) => {
    switch (result) {
      ...
      case 'testing': return 'text-yellow-600 bg-yellow-50 animate-pulse';
  ...
  return (
    <Card className="p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-600">Test WebSocket reconnection behavior and error handling.</p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="text-center">
          <div className="text-2xl font-bold">{ws.latency ? `${ws.latency}ms` : '--'}</div>
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-3 gap-2 text-sm">
          <div className={cn('p-2 rounded', getResultColor(testResults.reconnection))}>
            <span className="mr-2">{getResultIcon(testResults.reconnection)}</span>
            Reconnection
          </div>
          <div className={cn('p-2 rounded', getResultColor(testResults.errorHandling))}>
            <span className="mr-2">{getResultIcon(testResults.errorHandling)}</span>
            Error Handling
          </div>
          <div className={cn('p-2 rounded', getResultColor(testResults.heartbeat))}>
              ...
              <div key={index} className="flex justify-between items-center p-1 bg-gray-50 rounded">
                ...
                <div className="flex space-x-2">
                  <span className={cn(
                    ...
                    entry.status === 'connected' && 'bg-green-100 text-green-700',
                    entry.status === 'disconnected' && 'bg-gray-100 text-gray-700',
                    entry.status === 'error' && 'bg-red-100 text-red-700',
                    entry.status === 'connecting' && 'bg-yellow-100 text-yellow-700',
                    entry.status === 'reconnecting' && 'bg-orange-100 text-orange-700'
                  )}>
                    {entry.status}
                  </span>
                  {entry.details && (
      ...
      <div className="flex space-x-2 pt-4 border-t">
        <Button size="sm" variant="outline" onClick={ws.connect} disabled={ws.isConnected}>
          Connect
        </Button>
        <Button size="sm" variant="outline" onClick={ws.disconnect} disabled={!ws.isConnected}>
          Disconnect
        </Button>
        <Button size="sm" variant="outline" onClick={ws.reconnect}>
          Force Reconnect
        </Button>
        <Button size="sm" variant="outline" onClick={ws.sendTest} disabled={!ws.isConnected}>
          Send Test
        </Button>
        <Button 
          size="sm" 
          ...
          onClick={() => {
            ...
            setTestResults({ reconnection: null, errorHandling: null, heartbeat: null });
          }}
        >
          Clear
        </Button>
