...
func (p *PrometheusMetricsCollector) StatsJobStarted(jobType string, volumeID string) {
	p.statsJobsStartedTotal.WithLabelValues(jobType, volumeID).Inc()
}

// StatsJobCompleted records when a stats job completes successfully
func (p *PrometheusMetricsCollector) StatsJobCompleted(jobType string, volumeID string, duration time.Duration, recordsProcessed int) {
	p.statsJobsCompletedTotal.WithLabelValues(jobType, volumeID).Inc()
	p.statsJobDurationHistogram.WithLabelValues(jobType, volumeID).Observe(duration.Seconds())
}

// StatsJobFailed records when a stats job fails
func (p *PrometheusMetricsCollector) StatsJobFailed(jobType string, volumeID string, duration time.Duration, errorType string) {
	p.statsJobsFailedTotal.WithLabelValues(jobType, volumeID, errorType).Inc()
	p.statsJobDurationHistogram.WithLabelValues(jobType, volumeID).Observe(duration.Seconds())
}

// UpdateStatsJobQueueDepth updates the stats job queue depth
func (p *PrometheusMetricsCollector) UpdateStatsJobQueueDepth(depth int) {
	p.statsJobQueueDepthGauge.Set(float64(depth))
}

// SetStatsServiceStatus updates stats service status
func (p *PrometheusMetricsCollector) SetStatsServiceStatus(enabled bool) {
	if enabled {
		p.statsServiceStatus.Set(1)
	} else {
		p.statsServiceStatus.Set(0)
	}
}
