...
SELECT 
    CASE 
        ...
        ELSE CAST((n * sum_xy - sum_x * sum_y) AS REAL) / CAST((n * sum_x2 - sum_x * sum_x) AS REAL)
    END as slope,
    n as data_points
FROM slope_calc;
...
SELECT 
    volume_id,
    date(snapshot_date, 'weekday 1', '-7 days') as week_start,
...
WHERE snapshot_type = 'daily' 
    AND snapshot_date >= date('now', '-90 days')
    AND snapshot_date < date('now', '-7 days')
GROUP BY volume_id, date(snapshot_date, 'weekday 1', '-7 days')
...
FROM volume_sizes 
WHERE volume_id = ? AND is_valid = true
