...
export const EnhancedScanProgress: React.FC<EnhancedScanProgressProps> = ({
  ...
  const fileContext = useMemo((): EnhancedFileContext => {
    ...
    return {
      ...
      currentDirectory: currentItem ? currentItem.substring(0, currentItem.lastIndexOf('/')) : undefined,
      processingStep: itemParts[3] || 'Processing',
      // Batch information (if available in phase data)
      currentBatch: currentPhase.items_total > 1000 ? {
        number: Math.floor(currentPhase.items_processed / 1000) + 1,
        totalBatches: Math.ceil(currentPhase.items_total / 1000),
        filesInBatch: Math.min(1000, currentPhase.items_total - Math.floor(currentPhase.items_processed / 1000) * 1000),
        batchProgress: ((currentPhase.items_processed % 1000) / Math.min(1000, currentPhase.items_total - Math.floor(currentPhase.items_processed / 1000) * 1000)) * 100
      } : undefined,
  ...
  const getFileTypeIcon = (fileType?: string, fileName?: string) => {
    ...
    if (fileType?.startsWith('audio/') || ['mp3', 'wav', 'flac', 'aac', 'm4a'].includes(extension || '')) {
  ...
  const performanceMetrics = useMemo(() => {
    ...
    return {
      ...
      efficiency: stats.error_rate < 0.01 ? 'Excellent' : stats.error_rate < 0.05 ? 'Good' : 'Needs Attention',
      eta: stats.estimated_remaining_seconds,
      memoryPressure: stats.memory_usage_bytes ? (stats.memory_usage_bytes / (1024 * 1024 * 1024)).toFixed(1) + ' GB' : undefined,
      cpuUsage: stats.cpu_usage_percent ? Math.round(stats.cpu_usage_percent) + '%' : undefined,
    };
    ...
    <div className={clsx('space-y-4', className)} data-testid={testId}>
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-blue-700">
                  Currently in <strong>{currentPhase.phase_name.replace('_', ' ')}</strong>
                  {nextPhase && (
                    <span className="ml-2 text-blue-600">
                      → Next: <strong>{nextPhase.phase_name.replace('_', ' ')}</strong>
                    </span>
                  )}
                </p>
              </div>
      ...
      {showFileContext && fileContext.currentFile && (
        <div className="bg-white border border-gray-200 rounded-lg">
          ...
          >
            <div className="flex items-center space-x-3">
              {getFileTypeIcon(fileContext.currentFileType, fileContext.currentFile)}
              <div>
                <h4 className="font-medium text-gray-900">Currently Processing</h4>
                <p className="text-sm text-gray-500">{fileContext.processingStep || 'Processing file'}</p>
              </div>
            </div>
            {expandedSections.fileContext ? <ChevronUp className="w-5 h-5 text-gray-400" /> : <ChevronDown className="w-5 h-5 text-gray-400" />}
          </div>
          ...
          {expandedSections.fileContext && (
            <div className="border-t border-gray-100 p-4 space-y-3">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <div className="text-xs font-medium text-gray-500 uppercase tracking-wide">File Path</div>
                  <div className="text-sm text-gray-900 font-mono truncate" title={fileContext.currentFile}>
                ...
                {fileContext.currentFileType && (
                  <div>
                    <div className="text-xs font-medium text-gray-500 uppercase tracking-wide">File Type</div>
                    <div className="text-sm text-gray-900">{fileContext.currentFileType}</div>
                  </div>
                )}
                {fileContext.currentDirectory && (
                  <div>
                    <div className="text-xs font-medium text-gray-500 uppercase tracking-wide">Directory</div>
                    <div className="text-sm text-gray-900 font-mono truncate" title={fileContext.currentDirectory}>
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">
                  Batch {fileContext.currentBatch.number} of {fileContext.currentBatch.totalBatches}
                </p>
              </div>
            </div>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-3 gap-4 text-sm">
                  <div className="text-center">
                    <div className="font-medium text-gray-900">{fileContext.currentBatch.filesInBatch}</div>
                    <div className="text-xs text-gray-500">Files in Batch</div>
                  </div>
                  <div className="text-center">
                    <div className="font-medium text-gray-900">{fileContext.currentBatch.number}</div>
                    <div className="text-xs text-gray-500">Current Batch</div>
                  </div>
                  <div className="text-center">
                    <div className="font-medium text-gray-900">{fileContext.currentBatch.totalBatches}</div>
          ...
          >
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">
                  {Math.round(performanceMetrics.throughput)} files/sec • {performanceMetrics.efficiency} efficiency
                </p>
              </div>
            </div>
            ...
            <div className="border-t border-gray-100 p-4">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                <div className="text-center p-3 bg-green-50 rounded-lg">
                  <div className="font-bold text-green-700 text-lg">{Math.round(performanceMetrics.throughput)}</div>
                  <div className="text-green-600 text-xs">Files/Second</div>
                </div>
                <div className="text-center p-3 bg-blue-50 rounded-lg">
                  <div className="font-bold text-blue-700 text-lg">{formatBytes(performanceMetrics.dataRate)}</div>
                ...
                <div className="text-center p-3 bg-gray-50 rounded-lg">
                  <div className="font-bold text-gray-700 text-lg">{performanceMetrics.efficiency}</div>
                  <div className="text-gray-600 text-xs">Efficiency</div>
                </div>
                {performanceMetrics.memoryPressure && (
                  <div className="text-center p-3 bg-orange-50 rounded-lg">
                    <div className="font-bold text-orange-700 text-lg">{performanceMetrics.memoryPressure}</div>
                    <div className="text-orange-600 text-xs">Memory Usage</div>
                  </div>
                )}
                {performanceMetrics.cpuUsage && (
                  <div className="text-center p-3 bg-red-50 rounded-lg">
                    <div className="font-bold text-red-700 text-lg">{performanceMetrics.cpuUsage}</div>
          ...
          >
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">Latest file processing activity</p>
              </div>
            </div>
            {expandedSections.recentActivity ? <ChevronUp className="w-5 h-5 text-gray-400" /> : <ChevronDown className="w-5 h-5 text-gray-400" />}
          </div>
              ...
              <div className="space-y-2 max-h-40 overflow-y-auto">
                {fileContext.recentFiles?.map((file, index) => (
                  <div key={index} className="flex items-center space-x-3 text-sm">
                    ...
                    <span className="font-mono text-gray-700 truncate">{file}</span>
                    <span className="text-xs text-gray-500">just now</span>
                  </div>
                ))}
                {progress.recent_errors?.slice(0, 3).map((error, index) => (
                  <div key={`error-${index}`} className="flex items-center space-x-3 text-sm">
                    <AlertTriangle className="w-4 h-4 text-red-500 flex-shrink-0" />
                    {getFileTypeIcon(undefined, error.item_name)}
                    <span className="font-mono text-red-700 truncate">{error.item_name}</span>
                    <span className="text-xs text-red-500">{error.error_message}</span>
                  </div>
                ))}
                {(!fileContext.recentFiles?.length && !progress.recent_errors?.length) && (
                  <div className="text-center text-gray-500 py-4">
                    <Activity className="w-6 h-6 mx-auto mb-2 animate-pulse" />
                    <p className="text-sm">Monitoring file processing activity...</p>
                  </div>
                )}
              </div>
            </div>
      ...
      {progress.recent_errors && progress.recent_errors.length > 0 && (
        <div className="bg-red-50 border border-red-200 rounded-lg p-4">
          ...
          <div className="space-y-3 max-h-32 overflow-y-auto">
            {progress.recent_errors.slice(0, 3).map((error, index) => (
              <div key={index} className="bg-white/70 border border-red-200 rounded-lg p-3">
                <div className="flex items-start space-x-3">
                  {getFileTypeIcon(undefined, error.item_name)}
                  <div className="flex-1 min-w-0">
                    <div className="font-medium text-red-900 truncate">{error.item_name}</div>
                    <div className="text-sm text-red-700">{error.error_message}</div>
                    <div className="text-xs text-red-600 mt-1">
                      {error.component} • {error.operation} • {new Date(error.occurred_at).toLocaleTimeString()}
                    </div>
                  </div>
                  <Badge variant="error" size="sm">{error.severity}</Badge>
                </div>
              </div>
            ))}
          </div>
