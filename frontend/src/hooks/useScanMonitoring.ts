...
export const useScanMonitoring = (
  ...
  const convertToScanData = useCallback((event: any): ScanData => {
    return {
      context: {
        ...
        volumeName: event.volume_name || event.volume_id,
        volumePath:
          event.volume_path || `/var/lib/docker/volumes/${event.volume_id}`,
  ...
  const startScan = useCallback(
    async (volumeId: string, options: any = {}) => {
      console.log(
        '[useScanMonitoring] Starting scan for volume:',
        volumeId,
        'options:',
        options,
      );
      try {
        ...
        console.log(
          '[useScanMonitoring] Response status:',
          response.status,
          response.statusText,
        );
        ...
        if (!response.ok) {
          ...
          throw new Error(
            `Failed to start scan: ${response.statusText} - ${errorText}`,
          );
        }

        const data = await response.json();
        console.log(
          '[useScanMonitoring] Scan started successfully, response data:',
          data,
        );
  ...
  const cancelScan = useCallback(
    async (scanId: string) => {
      ...
      setScanOperations((prev) => prev.filter((s) => s.scanId !== scanId));
    },
    [setScanOperations],
  );
  ...
  const getScanData = useCallback(
    (scanId: string): ScanData | undefined => {
