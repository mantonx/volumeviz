...
export class AdaptiveLoadingService {
  ...
  getLoadingParams(component: 'treemap' | 'sunburst' | 'list' | 'explorer'): Record<string, any> {
    ...
    return {
      ...
      prefetchDistance: adaptations.prefetchDistance || baseParams.prefetchDistance,
  ...
  private detectDeviceCapabilities(): DeviceCapabilities {
    ...
    const networkSpeed = this.mapNetworkSpeed(connection?.effectiveType || '4g');
    ...
    const isLowEndDevice = memory < 4 || cores < 4 || networkSpeed === 'slow-2g' || networkSpeed === '2g';
  ...
  private mapNetworkSpeed(effectiveType: string): DeviceCapabilities['networkSpeed'] {
  ...
  private saveUserBehavior(): void {
    try {
      localStorage.setItem('volumeviz_user_behavior', JSON.stringify(this.userBehavior));
    } catch (error) {
  ...
  private selectOptimalStrategy(): void {
    ...
    if (compatibleStrategies.length === 0) {
      ...
      const fallback = this.config.strategies.find(s => s.name === this.config.fallbackStrategy);
  ...
  private isStrategyCompatible(strategy: LoadingStrategy): boolean {
    ...
    if (conditions.networkSpeed && conditions.networkSpeed !== 'auto') {
      if (conditions.networkSpeed === 'fast' && !this.isFastNetwork()) return false;
      if (conditions.networkSpeed === 'slow' && this.isFastNetwork()) return false;
    }

    // Check device memory
    if (conditions.deviceMemory) {
      if (conditions.deviceMemory === 'high' && deviceCapabilities.memory < 8) return false;
      if (conditions.deviceMemory === 'medium' && (deviceCapabilities.memory < 4 || deviceCapabilities.memory >= 8)) return false;
      if (conditions.deviceMemory === 'low' && deviceCapabilities.memory >= 4) return false;
    }

    // Check CPU cores
    if (conditions.cpuCores) {
      if (conditions.cpuCores === 'many' && deviceCapabilities.cores < 8) return false;
      if (conditions.cpuCores === 'few' && deviceCapabilities.cores >= 4) return false;
    }

    // Check battery level
    if (conditions.batteryLevel === 'low' && deviceCapabilities.batteryLevel > 0.2) return false;
  ...
  private calculateStrategyScore(strategy: LoadingStrategy): number {
    ...
    if (metrics && metrics.length >= 5) {
      ...
      const avgDuration = successful.reduce((sum, m) => sum + m, 0) / successful.length;
  ...
  private evaluateAndAdapt(): void {
    ...
    const currentPerf = this.getStrategyPerformance(this.currentStrategy?.name || '');
  ...
  private getStrategyPerformance(strategyName: string): { successRate: number; avgDuration: number } {
    ...
    return {
      successRate: successful.length / metrics.length,
      avgDuration: successful.reduce((sum, m) => sum + m, 0) / successful.length,
    };
  }
  ...
  private updateVisualizationPreference(type: 'treemap' | 'sunburst' | 'list'): void {
    ...
    this.userBehavior.mostUsedVisualization = Object.entries(vizCounts)
  ...
  private updateInteractionFrequency(duration: number): void {
    ...
    if (this.userBehavior.averageSessionDuration < 60000) { // < 1 minute
      this.userBehavior.interactionFrequency = 'low';
    } else if (this.userBehavior.averageSessionDuration < 300000) { // < 5 minutes
    ...
    if ('memory' in performance) {
      const checkMemory = () => {
        ...
        if (memInfo.usedJSHeapSize / memInfo.totalJSHeapSize > 0.9) {
          console.warn('High memory usage detected, switching to low-resource strategy');
          const lowResourceStrategy = this.config.strategies.find(s => s.name === 'low-resource');
