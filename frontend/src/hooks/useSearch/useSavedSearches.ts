...
export function useSavedSearches() {
  ...
  const [currentSavedSearch, setCurrentSavedSearch] = useAtom(
    currentSavedSearchAtom,
  );
  ...
  const fetchSavedSearches = useCallback(
    async (params?: { page?: number; perPage?: number; tags?: string[] }) => {
  ...
  const getSavedSearch = useCallback(
    async (id: number): Promise<SavedSearch> => {
      try {
        setError(null);
  ...
  const duplicateSavedSearch = useCallback(
    async (id: number, newName?: string): Promise<SavedSearch> => {
      try {
