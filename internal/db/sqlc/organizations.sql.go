...
func (q *Queries) AcceptOrganizationInvitation(ctx context.Context, arg AcceptOrganizationInvitationParams) error {
	_, err := q.db.Exec(ctx, acceptOrganizationInvitation, arg.ID, arg.AcceptedBy)
	return err
}

const cancelOrganizationInvitation = `-- name: CancelOrganizationInvitation :exec
UPDATE organization_invitations
SET status = 'cancelled'
WHERE id = $1
`

func (q *Queries) CancelOrganizationInvitation(ctx context.Context, id int64) error {
	_, err := q.db.Exec(ctx, cancelOrganizationInvitation, id)
	return err
}
...
const getOrganizationInvitationByToken = `-- name: GetOrganizationInvitationByToken :one
...
func (q *Queries) GetOrganizationInvitationByToken(ctx context.Context, token string) (OrganizationInvitations, error) {
	row := q.db.QueryRow(ctx, getOrganizationInvitationByToken, token)
...
func (q *Queries) ListOrganizationInvitations(ctx context.Context, arg ListOrganizationInvitationsParams) ([]OrganizationInvitations, error) {
	rows, err := q.db.Query(ctx, listOrganizationInvitations, arg.OrganizationID, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
