...
import { searchApi, type SearchFilesRequest, type SearchFilesResponse } from '@/api/search';
import {
  searchQueryAtom,
  searchResultsAtom,
...
export function useSearch() {
  ...
  const buildSearchRequest = useCallback((baseQuery: SearchFilesRequest): SearchFilesRequest => {
  ...
  const executeSearch = useCallback(async (searchQuery?: SearchFilesRequest) => {
    ...
    try {
      ...
      const response: SearchFilesResponse = await searchApi.searchFiles(fullRequest);
  ...
  }, [buildSearchRequest, addToHistory, setLoading, setError, setResults, 
      setTotalCount, setTotalPages, setQueryTime, setActiveFilters, setCurrentPage, 
      setPerPage]);

  // Search with text query
  const searchFiles = useCallback(async (textQuery: string, options?: Partial<SearchFilesRequest>) => {
  ...
  // Change page size
  const changePageSize = useCallback(async (newPerPage: number) => {
  ...
  // Change sorting
  const changeSorting = useCallback(async (sort: string, order: 'asc' | 'desc' = 'asc') => {
    if (!lastSearchQueryRef.current) return;
