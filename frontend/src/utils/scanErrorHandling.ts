...
import { getErrorMessage, getErrorDetails, isApiError, getHttpStatusCode } from './errorHandling';

// Scan-specific error types
export interface ScanError {
  error_type: 'EnrichmentError' | 'FileAccessError' | 'FilesystemError' | 'NetworkError' | 'ResourceError' | 'ValidationError' | 'TimeoutError';
  error_category: 'metadata_extraction' | 'file_system' | 'disk_io' | 'network' | 'permissions' | 'validation' | 'timeout' | 'memory';
...
const SCAN_ERROR_SUGGESTIONS: Record<string, Record<string, string>> = {
  'FileAccessError': {
    'permissions': 'Check file permissions or run scan as administrator. Ensure the volume is mounted and accessible.',
    'file_system': 'Verify the file exists and the storage device is properly connected.',
  },
  'FilesystemError': {
    'disk_io': 'Check disk health with fsck (Linux/Mac) or chkdsk (Windows). Consider backing up data immediately.',
    'file_system': 'Try remounting the volume or check for disk errors. This may indicate hardware failure.',
  },
  'EnrichmentError': {
    'metadata_extraction': 'File may be corrupted or in an unsupported format. Consider updating media libraries or skipping this file.',
  },
  'NetworkError': {
    'network': 'Check network connectivity if scanning remote volumes. Ensure stable connection to network storage.',
  },
  'ResourceError': {
    'memory': 'Close other applications to free memory, or split large scans into smaller batches.',
    'disk_space': 'Free up disk space or change scan output directory to a drive with more space.',
  },
  'TimeoutError': {
    'timeout': 'Large files may need more time. Consider increasing timeout settings or scanning during off-peak hours.',
  },
  'ValidationError': {
    'validation': 'File appears corrupted or damaged. Try re-downloading or restoring from backup if available.',
  },
};
...
export function formatScanErrorForDisplay(error: ScanError): {
  ...
  const baseMessage = SCAN_ERROR_MESSAGES[error.error_type]?.[error.error_category] || error.error_message;
  const suggestion = SCAN_ERROR_SUGGESTIONS[error.error_type]?.[error.error_category];
  const operationContext = SCAN_OPERATION_CONTEXT[error.operation] || error.operation;
  ...
  const retryable = error.error_type !== 'ValidationError' && error.error_type !== 'FilesystemError';
 ...
 */
export function formatApiErrorForScan(error: any, context?: {
  phase?: string;
  operation?: string;
  volumeName?: string;
  fileName?: string;
  ...
  if (context) {
    const parts: string[] = [];
    if (context.phase) {
      parts.push(SCAN_PHASE_CONTEXT[context.phase] || `During ${context.phase}`);
    }
    if (context.operation) {
      parts.push(SCAN_OPERATION_CONTEXT[context.operation] || context.operation);
    }
  ...
  if (statusCode) {
    switch (statusCode) {
      case 400:
        ...
        message = 'The scan request contains invalid parameters. Please check your scan settings and try again.';
      ...
      case 403:
        ...
        message = 'You don\'t have permission to scan this volume or access these files.';
        suggestion = 'Contact your administrator or check volume access permissions.';
        retryable = false;
        break;
      case 404:
        title = 'Volume Not Found';
        message = 'The specified volume could not be found or may have been unmounted.';
        suggestion = 'Verify the volume is still connected and accessible, then refresh the volume list.';
        variant = 'info';
        break;
      case 409:
        title = 'Scan Already Running';
        message = 'A scan is already in progress for this volume.';
        suggestion = 'Wait for the current scan to complete or cancel it before starting a new one.';
        variant = 'warning';
        retryable = false;
        break;
      case 413:
        title = 'Volume Too Large';
        message = 'The volume is too large to scan with current system resources.';
        suggestion = 'Consider scanning specific directories or upgrading system resources.';
        retryable = false;
        break;
      case 422:
        title = 'Invalid Volume Data';
        message = 'The volume contains invalid or corrupted data that prevents scanning.';
        suggestion = 'Check disk integrity or try scanning specific subdirectories.';
        retryable = false;
        break;
      case 429:
        title = 'Too Many Scan Requests';
        message = 'You have reached the limit for concurrent scans.';
        suggestion = 'Wait for existing scans to complete before starting new ones.';
        variant = 'warning';
        break;
      case 500:
  ...
  if (isApiError(error)) {
    switch (error.error.code) {
      case 'VOLUME_NOT_ACCESSIBLE':
        ...
        suggestion = 'Check if the volume is mounted and you have read permissions.';
        break;
      ...
      case 'INSUFFICIENT_RESOURCES':
        ...
        suggestion = 'Close other applications or try scanning smaller directories.';
        break;
      case 'VOLUME_TOO_LARGE':
        title = 'Volume Size Limit Exceeded';
        message = 'The volume is too large for the current scan configuration.';
        suggestion = 'Consider scanning specific directories or adjusting scan settings.';
        retryable = false;
        break;
    }
  }
...
export function getScanStatusMessage(
  ...
  switch (status) {
    case 'pending':
      return {
        message: 'Scan Queued',
        description: 'Your scan is waiting to begin. It will start once system resources are available.',
        variant: 'info',
      };
    case 'running':
      const progressText = context?.progress ? ` (${context.progress}%)` : '';
      const phaseText = context?.phase ? ` - ${SCAN_PHASE_CONTEXT[context.phase] || context.phase}` : '';
      const fileText = context?.fileName ? ` - Processing: ${context.fileName}` : '';
      return {
    ...
    case 'completed':
      return {
        ...
        description: 'Your volume has been fully scanned and indexed. All files have been analyzed.',
        variant: 'success',
      };
    case 'failed':
      return {
        message: 'Scan Failed',
        description: 'The scan encountered errors and could not complete. Check error details below.',
        variant: 'error',
      };
    case 'cancelled':
      return {
        message: 'Scan Cancelled',
        description: 'The scan was cancelled by user request. Partial results may be available.',
        variant: 'warning',
      };
    case 'paused':
      return {
        message: 'Scan Paused',
        description: 'The scan is temporarily paused. Resume to continue scanning from where it left off.',
        variant: 'warning',
      };
    default:
      return {
        message: 'Unknown Status',
        description: 'The scan status is unclear. Please refresh or check system logs.',
        variant: 'warning',
      };
  }
}
 ...
 */
export function formatBatchErrorContext(error: ScanError, batchInfo?: {
  currentBatch: number;
  totalBatches: number;
  filesInBatch: number;
...
export function isCriticalScanError(error: ScanError): boolean {
  return (
    ...
    (error.error_type === 'FilesystemError' && error.error_category === 'disk_io') ||
    (error.error_type === 'ResourceError' && error.error_category === 'memory') ||
    error.retry_count >= 3
  );
}
...
export function getRecommendedScanAction(error: ScanError): {
  ...
  if (isCriticalScanError(error)) {
    return {
      action: 'abort',
      reason: 'Critical error detected that may affect system stability or data integrity',
      immediate: true,
    };
  }
  ...
  if (error.error_type === 'ResourceError') {
    return {
      action: 'pause',
      reason: 'System resources are low - pausing to prevent system instability',
      immediate: true,
    };
  }
 ...
 */
export function scanErrorToSummaryItem(scanError: ScanError, index: number = 0): {
  id: string;
  message: string;
  code?: string;
  severity: 'low' | 'medium' | 'high' | 'critical';
  category: 'permission' | 'network' | 'filesystem' | 'validation' | 'timeout' | 'resource' | 'unknown';
  timestamp: Date;
  context?: {
    phase?: string;
    path?: string;
  ...
  const categoryMap: Record<string, 'permission' | 'network' | 'filesystem' | 'validation' | 'timeout' | 'resource' | 'unknown'> = {
 ...
 */
export function scanErrorsToSummaryItems(scanErrors: ScanError[]): ReturnType<typeof scanErrorToSummaryItem>[] {
