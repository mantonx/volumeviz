...
import type { ScanManagerDashboardProps, ScanOperation } from './ScanManagerDashboard.types';
...
export const ScanManagerDashboard: React.FC<ScanManagerDashboardProps> = ({
  ...
  return (
    <div className={className} data-testid={testId}>
      {systemMetrics && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <MetricCard
            ...
            trendValue={systemMetrics.activeScans > 0 ? `${systemMetrics.activeScans} running` : undefined}
          />
          <MetricCard
            title="Queued Scans"
            value={systemMetrics.queuedScans}
            trend={systemMetrics.queuedScans > 5 ? 'down' : undefined}
            trendValue={systemMetrics.queuedScans > 0 ? `${systemMetrics.queuedScans} waiting` : undefined}
          />
          <MetricCard
            title="Total Files"
            value={formatNumber(systemMetrics.totalFilesScanned)}
            trend="up"
            trendValue={systemMetrics.averageScanSpeed > 0 ? `${formatNumber(systemMetrics.averageScanSpeed)} files/s` : undefined}
          />
          <MetricCard
            title="System Load"
            value={systemMetrics.systemLoad ? `${systemMetrics.systemLoad.toFixed(1)}%` : 'N/A'}
            trend={systemMetrics.systemLoad && systemMetrics.systemLoad > 80 ? 'down' : 'up'}
            variant={systemMetrics.systemLoad && systemMetrics.systemLoad > 80 ? 'error' : 'success'}
          />
        </div>
      )}
      ...
      {systemMetrics && (systemMetrics.memoryUsage || systemMetrics.diskIORate) && (
        <Card>
            ...
            {systemMetrics.memoryUsage !== undefined && (
              <div>
                ...
                <ProgressBar
                  ...
                  max={100}
                  variant={systemMetrics.memoryUsage > 80 ? 'error' : systemMetrics.memoryUsage > 60 ? 'warning' : 'primary'}
                  size="sm"
                />
              </div>
            )}
            {systemMetrics.diskIORate !== undefined && (
              <div>
                <div className="flex items-center justify-between text-sm mb-1">
                  ...
                  <span className="font-medium">{formatBytes(systemMetrics.diskIORate)}/s</span>
                </div>
                <ProgressBar
                  value={Math.min(systemMetrics.diskIORate / (1024 * 1024 * 100) * 100, 100)}
                  max={100}
                  variant="primary"
                  size="sm"
                />
