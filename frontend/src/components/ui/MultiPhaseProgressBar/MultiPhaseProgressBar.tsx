...
export const MultiPhaseProgressBar: React.FC<MultiPhaseProgressBarProps> = ({
  ...
  const [historicalLoadError, setHistoricalLoadError] = useState<string | null>(
    null,
  );
  ...
  useEffect(() => {
    const fetchHistoricalData = async () => {
      ...
      try {
        ...
        console.log('MultiPhaseProgressBar: Phases:', data.phases);
        console.log('MultiPhaseProgressBar: Phase configs:', data.phases?.map(p => ({ 
          name: p.phase_name, 
          config: DEFAULT_PHASE_CONFIG[p.phase_name as keyof typeof DEFAULT_PHASE_CONFIG] 
        })));
  ...
  useEffect(() => {
    ...
    const handleScanStart = (data: any) => {
      ...
      // Filter by volume_id
      if (data.volume_id === volumeId || (scanId && data.scan_id === scanId)) {
        // Clear old progress data when new scan starts to show fresh state
        setProgress(null);
        setLastUpdateTime(new Date());
    ...
    for (const phase of progress.phases) {
      const phaseConfig =
        DEFAULT_PHASE_CONFIG[
          phase.phase_name as keyof typeof DEFAULT_PHASE_CONFIG
        ];
      if (phaseConfig) {
  ...
  const getPhaseConfig = useCallback(
    (phaseName: string) => {
      const defaultConfig =
        DEFAULT_PHASE_CONFIG[phaseName as keyof typeof DEFAULT_PHASE_CONFIG];
      if (!defaultConfig) return null;

      return {
        ...defaultConfig,
        label:
          phaseLabels[phaseName as keyof typeof phaseLabels] ||
          defaultConfig.label,
        description:
          phaseDescriptions[phaseName as keyof typeof phaseDescriptions] ||
  ...
  const renderPhase = (phase: ScanPhaseProgress, index: number) => {
    ...
    if (!phaseConfig) {
      console.log('MultiPhaseProgressBar: Missing config for phase:', phase.phase_name, phase);
      return null;
    }
    ...
    return (
      <div
        ...
        className={clsx(
          ...
          {
            'bg-blue-50 dark:bg-blue-950/30 border-blue-200 dark:border-blue-800': isActive,
            'bg-green-50 dark:bg-green-950/30 border-green-200 dark:border-green-800': isCompleted,
        ...
        role="region"
        aria-label={`${phaseConfig.label} progress`}
      ...
      >
          ...
          <div className="flex items-center space-x-2">
            ...
            <span
              className={clsx(
                ...
                size === 'lg' && 'text-lg',
                {
                  'text-blue-900 dark:text-blue-100': isActive,
                  'text-green-900 dark:text-green-100': isCompleted,
                  'text-red-900 dark:text-red-100': isFailed,
                  'text-gray-900 dark:text-gray-100': !isActive && !isCompleted && !isFailed,
                },
              )}
              role="status"
              aria-live="polite"
        ...
        {showPhaseDescriptions && !compact && (
          <p
            ...
            aria-describedby={`${testId}-phase-${phase.phase_name}-description`}
            id={`${testId}-phase-${phase.phase_name}-description`}
          >
        ...
        {showDetailedMetrics && !compact && isActive && (
          <div
            className={clsx(
              'grid grid-cols-2 gap-2 text-xs text-gray-800 dark:text-gray-200',
              size === 'sm' && 'text-xs',
              size === 'lg' && 'text-sm',
            )}
            role="region"
          ...
          >
            ...
            {phase.bytes_processed > 0 && (
              <div>
                ...
                {formatBytes(phase.bytes_processed)}
                {phase.bytes_total > 0 &&
                  ` / ${formatBytes(phase.bytes_total)}`}
              </div>
            )}
            {phase.items_per_second > 0 && (
              <div>
                <span className="font-medium">Rate:</span>{' '}
                {Math.round(phase.items_per_second)}/sec
              </div>
            )}
            {phase.current_item && (
              <div className="col-span-2 space-y-1">
                <div className="truncate">
                  ...
                  {phase.current_item.split('|')[0] || phase.current_item}
                </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    {phase.current_item.split('|')[1] && (
                      <div>
                        ...
                        {formatBytes(
                          parseInt(phase.current_item.split('|')[1]),
                        )}
                      </div>
                    )}
        ...
        {showErrors && phase.error_message && (
          <div 
            className="text-red-900 dark:text-red-100 text-sm bg-red-100 dark:bg-red-900/20 p-2 rounded border border-red-300 dark:border-red-700"
            role="alert"
  ...
  if (!progress) {
    return (
      <div
        className={clsx('text-center py-4 text-gray-500', className)}
        data-testid={testId}
      >
        {isLoadingHistorical ? (
          <>
  ...
  return (
    <div className={clsx('w-full space-y-4', className)} data-testid={testId}>
      ...
      {showEstimatedTime &&
        !compact &&
        progress.estimated_end_time &&
        isScanning && (
          <div className="text-center text-sm text-gray-600">
            <Clock className="w-4 h-4 inline mr-1" />
            Estimated completion:{' '}
            {new Date(progress.estimated_end_time).toLocaleTimeString()}
          </div>
        )}
        ...
        <div className="bg-red-50 border border-red-200 rounded-lg p-3">
          ...
          <div className="space-y-2 max-h-32 overflow-y-auto">
            {progress.recent_errors.slice(0, 3).map((error, index) => (
              <div
                key={index}
                className="text-red-700 text-sm bg-white/50 p-2 rounded border"
              >
      ...
      {showDetailedMetrics && !compact && progress.performance_stats && (
        <div className="bg-gray-50 border border-gray-200 rounded-lg p-3">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm text-gray-600">
            ...
            <div>
              <span className="font-medium">Items/sec:</span>{' '}
              {Math.round(progress.performance_stats.overall_items_per_second)}
            </div>
            <div>
              <span className="font-medium">Data/sec:</span>{' '}
              {formatBytes(progress.performance_stats.overall_bytes_per_second)}
              /s
            </div>
          </div>
        </div>
