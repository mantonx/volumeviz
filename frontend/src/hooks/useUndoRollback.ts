...
export function useUndoRollback(
  options: UseUndoRollbackOptions,
): UseUndoRollbackReturn {
  ...
  const rollbackOperation = useCallback(
    async (
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : 'Failed to rollback operation';
        setError(errorMessage);
        if (onError) {
          onError(errorMessage);
        }
  ...
  const cleanupBackups = useCallback(
    async (retentionDays: number = 30) => {
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : 'Failed to cleanup backups';
        setError(errorMessage);
        if (onError) {
          onError(errorMessage);
        }
